...
一套完整的 agent 开发方法论，把推敲需求、TDD、子 agent 并行开发、代码审查打包成可组合的 skills。装好后 Claude 会先和你对齐需求、出方案、拆分任务再动手——而不是上来就写代码。详见 [Superpowers](/superpowers)。

### karpathy-guidelines

从 Karpathy 对 LLM 编程痛点的观察中提炼出的行为规范，只有 4 条原则约 70 行：先想再写、简洁优先、精准修改、目标驱动。装好后自动生效，让 Claude 改代码更克制、更靠谱。详见 [Karpathy Guidelines](/karpathy-guidelines)。
